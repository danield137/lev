...
class CsvResultSink:
    ...
    def _result_to_dict(self, result: McpEvaluationResult) -> dict[str, Any]:
        ...
        complex_fields = {
            'conversation', 'mcps', 'tool_calls_sequence', 
            'individual_scores'
        }
...
def create_csv_result_sink(suite_name: str) -> CsvResultSink:
    """Create a CSV result sink with timestamped filename."""
    epoch = int(time.time())
    filename = f"{suite_name}_results_{epoch}.csv"
    return CsvResultSink(filename)
