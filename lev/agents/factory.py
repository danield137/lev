...
from lev.dataset_loader import get_persona_system_prompt
...
def create_agent_from_provider(eval_config: Dict[str, Any], provider: LlmProvider) -> SimpleAgent:
    """Create an asker agent using a provider directly."""
    asker_persona = eval_config.get("asker_persona", "diligent_asker")
...
def create_reasoning_agent_from_provider(
    eval_config: Dict[str, Any], provider: LlmProvider, mcp_registry: Optional[McpClientRegistry] = None
) -> ReasoningAgent:
    """Create a reasoning agent using a provider directly."""
    mcp_clients = []
    if mcp_registry:
        mcp_clients = create_mcp_clients(eval_config, mcp_registry)
